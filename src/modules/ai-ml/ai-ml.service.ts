...
export class AiMlService {
  ...
  async getLanguagesForUser(latLongDto: LatLongDto) {
    try {
      ...
      let countryAndState: CountryAndStateDto = await this.getAddressService.getCountryAndState(latLongDto);
      if (countryAndState.state == 'Himachal Pradesh') {
        countryAndState.state = 'Punjab';
      }
